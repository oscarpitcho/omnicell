...
class GeneShuffle(torch.nn.Module):
        ...
        def forward(self, genes : torch.Tensor, mask=True):
            ...
            indexes = [_random_indexes(T) for _ in range(B)]
            ...
            genes = _take_indexes(genes, forward_indexes)
            if mask:
...
class MAE_Decoder(torch.nn.Module):
    ...
    def forward(self, features, backward_indexes, pert_features=None):
        ...
        features = _take_indexes(features, backward_indexes)
        ...
        mask[:, T:] = 1
        mask = _take_indexes(mask, backward_indexes)
